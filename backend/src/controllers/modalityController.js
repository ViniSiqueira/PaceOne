...
const deletarModality = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM modalidade WHERE id = $1 RETURNING *', [id]);
...
module.exports = { salvarModality, listModalitys, atualizarModality, deletarModality };
